...
- `6f91a06`, `bfef7cd`, and a following code-review hardening pass (2026-09-13) — MD5-backed cross-generation reuse for `aggregation_covering.py`: a generation can now safely reuse an unchanged position's output from the immediately previous generation instead of reprocessing it from zero, verified by (1) an exact match of the covering CSV's own content AND every referenced source file's own manifest MD5, not just filename/maxzoom, and (2) the previous generation's own output file actually existing on disk — never a bare skip, always a real copy plus a fresh `.done` manifest scoped to the current generation_id. Exists only because this project's own dirty-tracking history (bridge repo D51/D57/D69) found that skip-based cross-generation optimizations silently lose data when the baseline generation was itself incomplete; the MD5 layer specifically closes a second, independently-discovered gap (bridge repo D18/D35: a same-filename, same-byte-size source file whose content silently changed after a corruption fix). Also fixed in the same batch: `aggregation_merge.py`'s `merged-3857.tiff` write was non-atomic, so a crash mid-write could leave a truncated file that the function's own resume logic would mistake for a completed merge (bridge repo D120 Fable review item #4, D164). Full narrative: bridge repo D163/D164.

None of this generalizes to a plain-DEM upstream deployment the way section A's fixes do — `datatype`, `generation_id`, and the lineage tile type only exist because the Japan bridge project runs multiple source-tier generations against the same store and wanted a per-pixel provenance layer. Full narrative and rationale: `hfu/mapterhorn-japan-bridge`'s own `DECISIONS.md` D93-D164.
